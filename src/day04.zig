...
fn yearInRange(y: ?usize, min: usize, max: usize) bool {
    return y != null and y.? >= min and y.? <= max;
}

fn heightValid(hgt: []const u8) !bool {
    ...
    outer: while (lines.next()) |line| {
        ...
        while (fields.next()) |f| {
            ...
            const fieldValid = switch (fieldName) {
                .byr => val.len == 4 and yearInRange(try std.fmt.parseUnsigned(usize, val, 10), 1920, 2002),
                .iyr => val.len == 4 and yearInRange(try std.fmt.parseUnsigned(usize, val, 10), 2010, 2020),
                .eyr => val.len == 4 and yearInRange(try std.fmt.parseUnsigned(usize, val, 10), 2020, 2030),
                .hgt => try heightValid(val),
                .hcl => val.len == 7 and val[0] == '#' and blk: for (val[1..]) |c| {
                    if (!((c >= '0' and c <= '9') or (c >= 'a' and c <= 'f'))) break :blk false;
                } else true,
                .ecl => std.meta.stringToEnum(EyeColor, val) != null,
                .pid => val.len == 9 and blk: for (val[1..]) |c| {
